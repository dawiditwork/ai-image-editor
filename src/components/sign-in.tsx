...
const { mutate: signInEmail, isPending: signInEmailPending } = useSignInEmail({
  ...
  if ((error as { error?: { code?: string } }).error?.code === "EMAIL_NOT_VERIFIED") {
      toast.error(message, {
        action: {
...
<Card
  className={cn(
    "w-full max-w-lg border border-slate-200 bg-white shadow-[0_25px_60px_-15px_rgba(0,0,0,0.25)] rounded-3xl",
    className
  )}
>
  <CardHeader className="px-8 pt-8 pb-2">
    <CardTitle className="text-3xl font-bold text-slate-900">
      ...
      Sign in to access your projects and image tools.
    </p>
      </CardHeader>

  <CardContent className="px-8 pb-8">        <div className="flex flex-col gap-6">
          {socialPosition === "top" && (
            <>
  ...
  type="submit"
  disabled={isPending}
  className="h-12 w-full rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 font-semibold text-white hover:from-blue-700 hover:to-purple-700"
>
                    {isPending && <Spinner />}
